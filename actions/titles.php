...
// Get subtheme info
$sub = q(
    "SELECT st.*,t.name tname FROM subthemes st JOIN themes t ON t.id=st.theme_id WHERE st.id=? AND st.deleted_at IS NULL AND t.deleted_at IS NULL",
    [$sub_id]
)->fetch();
if (!$sub) {
...
if (empty($allowed_teacher_ids)) {
    $titles = q("SELECT * FROM quiz_titles WHERE subtheme_id=? AND owner_user_id IS NULL AND deleted_at IS NULL ORDER BY title", [$sub_id])->fetchAll();
} else {
    $placeholders = implode(',', array_fill(0, count($allowed_teacher_ids), '?'));
    $titles = q(
        "SELECT * FROM quiz_titles WHERE subtheme_id=? AND (owner_user_id IS NULL OR owner_user_id IN ($placeholders)) AND deleted_at IS NULL ORDER BY title",
